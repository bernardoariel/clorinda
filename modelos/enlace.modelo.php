...
class ModeloEnlace{
	...
	static public function mdlMostrarModificacionesEnlace($tabla, $item, $valor){
		...
		$stmt = Conexion::conectarEnlace()->prepare("SELECT * FROM $tabla WHERE $item = :$item and clorinda = 0 order by id desc limit 1");
	...
	static public function mdlSubirModificaciones($tabla,$datos){
		...
		$stmt = Conexion::conectarEnlace()->prepare("UPDATE $tabla SET clorinda = 1 WHERE $datos");
